...
def setup_bank(si):
    ...
    sand_location = si.find_image_cv2('python_bots/image_library/bucket_of_sand.png', threshold=1)
    soda_ash_location = si.find_image_cv2('python_bots/image_library/soda_ash.png', threshold=1)
    ...
    # Withdraw bucket of sand with quantity 13
    ...
    withdraw_14_location = si.find_image_cv2('python_bots/image_library/withdraw-14.png', threshold=0.94)
    ...
    # Withdraw soda ash with quantity 14
    print("Withdrawing soda ash...")
    pyautogui.moveTo(soda_ash_location[0], soda_ash_location[1])
    time.sleep(random.uniform(0.2, 0.4))
    pyautogui.click(button='right')
    time.sleep(random.uniform(1.2, 1.5))
    ...
    if withdraw_14_location:
        ...
        time.sleep(random.uniform(0.2, 0.4))
        pyautogui.click()
        print("Clicked 'Withdraw-14' for soda ash")
    else:
        print("Could not find withdraw-14 option for soda ash. Trying withdraw-X option.")
        # Try withdraw-x option
        withdraw_x_location = si.find_image_cv2('python_bots/image_library/withdraw-X.png', threshold=0.85)
        if withdraw_x_location:
            pyautogui.moveTo(withdraw_x_location[0], withdraw_x_location[1])
            ...
            time.sleep(random.uniform(0.2, 0.4))
            pyautogui.press('enter')
            print("Entered withdraw amount: 14 for soda ash")
        else:
            print("Could not find withdraw options for soda ash. Stopping script.")
            return False
...
def main_loop(max_loops=50):
    ...
    for loop in range(1, max_loops + 1):
        ...
        time.sleep(random.uniform(7, 9))  # Wait for walk to furnace
        ...
        time.sleep(random.uniform(17, 21))
        ...
        withdraw_sand_success = si.click_image_cv2_without_moving("python_bots/image_library/bucket_of_sand.png",
                                                         confidence=1.0, offset_range=(0, 2))
