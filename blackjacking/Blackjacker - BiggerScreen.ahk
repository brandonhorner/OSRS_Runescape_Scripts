...
F4::
{
    ImageURL := images_local.knockout_option
    shade_variance := 20
    if ImageSearch(&foundX, &foundY, 0, 0, A_ScreenWidth, A_ScreenHeight, "*" shade_variance " " ImageURL)
    {
        ToolTip "Found image at: " foundX ", " foundY, X_TOOLTIP.2, Y_TOOLTIP.2, 2
        Sleep 200
        return(foundX, foundY)
    }
    else
    {
        ToolTip "Image not found.", X_TOOLTIP.2, Y_TOOLTIP.2, 2
        Sleep 200
    }
}
...
WaitForPickPocketAttempt()
{
    if WaitForImage(images_local.pickpocket_attempt, 100, coord.chat_bottom)
        return true
...
CheckAndUpdateStatus(&knockout_failure, &pickpocket_failure)
{
    if ImageExists(images_local.knockout_failure, coord.chat_bottom_2.x1, coord.chat_bottom_2.y1, coord.chat_bottom_2.x2, coord.chat_bottom_2.y2,)
        knockout_failure := true
    if ImageExists(images_local.pickpocket_failure, coord.chat_bottom_2.x1, coord.chat_bottom_2.y1, coord.chat_bottom_2.x2, coord.chat_bottom_2.y2,)
...
CheckIfStunned()
{
    ...
    if ImageSearchAndClick(images_local.imstunned, "top_left",,,,,,,5)
    {
        sleep_random(10, 10)
        return true
    }
...
HealthIsLow()
{
    if ImageExists(images_local.healthbar, 1400, 820, 1700, 850)
    {
        return false
    }
...
EatLobster()
{
    ...
    open_bag()
    sleep_random(500, 1500)
    if ImageSearchAndClick(images_local.lobster_cooked, "bag", "mouseover", "item") {
        sleep_random(500, 1500)
        Click("Left")
        sleep_random(500, 1500)
...
CheckIfFullOnMoneyBags()
{
    if ImageSearchAndClick(images_local.money_bag_full, "bag", "mouseover", "item") {
...
ClickMoneyBag()
{
    ...
    SendKey(1)
    if ImageExists(images_local.open_bag) {                                       ;ensure bag is open
        sleep_random(10, 100)
        if ImageSearchAndClick(images_local.money_bag, "bag", "mouseover", "item")
...
ClickKnockout() {
    ...
    WaitForImage(images_local.knockout_option, 200)
    if ImageSearchAndClick(images_local.knockout_option,, "mouseover", "option",,,,, 5) {
...
ClickPickpocket(timeout_ms := 300) {
    ...
    ; WaitForImage(images_local.pickpocket_option, timeout_ms)
    ; WaitForAnyImages([images_local.knockout_failure, images_local.knockout_success], timeout_ms)
    if ImageSearchAndClick(images_local.pickpocket_option, "under_mouse", "mouseover", "option") {
        Sleep 100
        Click("Left")
        return true
    }
...
RightClickNPC()
{
    ...
    if (true){ ;WaitForImage(images_local.menaphite_hovering_text, 700) {  ; if target is in sight
